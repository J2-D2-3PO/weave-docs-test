---
title: "Summarization using Chain of Density"
---

<Check>
  [![Open In Colab](/images/reference/gen_notebooks/wikipedia/commons/archive/d/d0/20221103151430-Google_Colaboratory_SVG_Logo.svg)](https://colab.research.google.com/github/wandb/weave/blob/master/docs/./notebooks/chain_of_density.ipynb)

  [Open in Colab](https://colab.research.google.com/github/wandb/weave/blob/master/docs/./notebooks/chain_of_density.ipynb)

  [![View in Github](/images/reference/gen_notebooks/wikipedia/commons/9/91/Octicons-mark-github.svg)](https://github.com/wandb/weave/blob/master/docs/./notebooks/chain_of_density.ipynb)

  [View in Github](https://github.com/wandb/weave/blob/master/docs/./notebooks/chain_of_density.ipynb)
</Check>
...
![](/images/reference/gen_notebooks/assets/images/eval_dash-f47531efecdbea87fbef3c3535ee040f.png)

## What is Chain of Density Summarization?[​](/reference/gen_notebooks/chain_of_density#what-is-chain-of-density-summarization "Direct link to What is Chain of Density Summarization?")

[![arXiv](/images/reference/gen_notebooks/badge/arXiv-2309.04269-b31b1b.svg)](https://arxiv.org/abs/2309.04269)
...
## Why use Weave?[​](/reference/gen_notebooks/chain_of_density#why-use-weave "Direct link to Why use Weave?")
...
## Set up the environment[​](/reference/gen_notebooks/chain_of_density#set-up-the-environment "Direct link to Set up the environment")
...
```
import ioimport osfrom datetime import datetime, timezoneimport anthropicimport requestsfrom pydantic import BaseModelfrom PyPDF2 import PdfReaderfrom set_env import set_envimport weaveset_env("WANDB_API_KEY")set_env("ANTHROPIC_API_KEY")weave.init("summarization-chain-of-density-cookbook")anthropic_client = anthropic.Anthropic(api_key=os.getenv("ANTHROPIC_API_KEY"))
```
...
## Create a Weave Model[​](/reference/gen_notebooks/chain_of_density#create-a-weave-model "Direct link to Create a Weave Model")

Now, let's wrap our summarization pipeline in a Weave Model:

![](/images/reference/gen_notebooks/assets/images/model-0b8edeb5b4f06c60bc02da20cf0a0ba1.png)

```
# Weave Modelclass ArxivChainOfDensityPipeline(weave.Model):    model: str = "claude-3-sonnet-20240229"    density_iterations: int = 3    @weave.op()    def predict(self, paper: ArxivPaper, instruction: str) -> dict:        text = load_pdf(paper.pdf_url)        result = chain_of_density_summarization(            text,            instruction,            model=self.model,            density_iterations=self.density_iterations,        )        return result
```
...
## Create a Weave Dataset and run evaluation[​](/reference/gen_notebooks/chain_of_density#create-a-weave-dataset-and-run-evaluation "Direct link to Create a Weave Dataset and run evaluation")

To evaluate our pipeline, we'll create a Weave Dataset and run an evaluation:

![](/images/reference/gen_notebooks/assets/images/dataset-39e8c0daa8f841378fdddad4c9c36fed.png)

```
# Create a Weave Datasetdataset = weave.Dataset(    name="arxiv_papers",    rows=[        {            "paper": arxiv_paper,            "instruction": "What was the approach to experimenting with different data mixtures?",        },    ],)weave.publish(dataset)
```
...
[![arXiv](/images/reference/gen_notebooks/badge/arXiv-2306.05685-b31b1b.svg)](https://arxiv.org/abs/2306.05685)

![](/images/reference/gen_notebooks/assets/images/eval_dash-f47531efecdbea87fbef3c3535ee040f.png)

```
# Define the scorer function@weave.op()def quality_scorer(instruction: str, output: dict) -> dict:    result = evaluate_summary(output["final_summary"], instruction)    return result
```

```
# Run evaluationevaluation = weave.Evaluation(dataset=dataset, scorers=[quality_scorer])arxiv_chain_of_density_pipeline = ArxivChainOfDensityPipeline()results = await evaluation.evaluate(arxiv_chain_of_density_pipeline)
```
...
## Conclusion[​](/reference/gen_notebooks/chain_of_density#conclusion "Direct link to Conclusion")
...
[View Full Report on W\&B](https://wandb.ai/wandb_fc/arxiv-reader/reports/Building-a-bot-to-summarize-arXiv-papers-as-PDFs-using-Anthrophic-and-W-B-Weave--Vmlldzo4Nzg0ODI4)

[Edit this page](https://github.com/wandb/weave/blob/master/docs/docs/reference/gen_notebooks/chain_of_density.md)
